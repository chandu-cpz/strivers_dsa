...
class Solution {
    public static void createTree(Node root0, ArrayList<Integer> v) {
        ...
        // Start from index 1 since root is already created
        for (int i = 1; i < v.size(); i++) {
            Node current = queue.poll(); // Remove and get the current parent node
            
            // Create left child
            Node leftNode = new Node(v.get(i));
            current.left = leftNode;
            queue.offer(leftNode);
            
            // Create right child if there are more nodes
            i++;
            if (i < v.size()) {
                Node rightNode = new Node(v.get(i));
